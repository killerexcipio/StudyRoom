...
export function useWhiteboard(whiteboardId: string) {
  const { tool, setTool, color } = useWhiteboardState();
  const { user } = useAuth();
  ...
  const saveShapes = useCallback(async (currentShapes: Shape[]) => {
    const { error } = await supabase
      .from("whiteboards")
      .update({ content: { shapes: currentShapes.map(s => ({...s, isEditing: undefined})) } })
      .eq("id", whiteboardId);
    if (error) {
      console.error("Failed to save shapes:", error);
    } else {
        broadcastShapes(currentShapes);
    }
  }, [supabase, whiteboardId, broadcastShapes]);
  ...
  useEffect(() => {
    ...
    const fetchShapes = async () => {
      ...
      const initialShapes = data?.content?.shapes || [];
